...
type Mutation {
    ...
    applyCampaign(
        ...
        # TODO(sqs): This is a way you can avoid race conditions when you update a campaign that had
        # a newer state from what you expected, like lastID in our settings API. Need to add details
        # here.
        lastSeq: Int
    ): Campaign!

    # Update a campaign identified by ID (not name) from a campaign spec and locally computed
    # changeset specs. The campaign is returned.
    #
    # This mutation is for callers that intend to rename the campaign or otherwise need/want to use
    # a stable, immutable ID to identify the campaign to update. Otherwise, callers can use the
    # applyCampaign mutation.
    updateCampaign(
        campaign: ID!
        spec: CampaignSpecInput!
        lastSeq: Int # TODO(sqs): See lastSeq above.
    ): Campaign!

    # TODO(sqs): do we really need create/apply/update? or can we combine them? I think we do, but we might be able to make them a single mutation with multiple params (although that might be more complex).
    #
    # Why is createCampaign needed? It would be weird if in the UI when you create a new empty campaign, if your campaign name is the same as one that exists, if it just took you to the existing one. In that case (given that the button is 'Create campaign'), it should show an error.
    #
    # Why is applyCampaign needed? This is the most obvious/general one. We could make the CLI decide whether to use createCampaign or updateCampaign by first seeing if the campaign existed, but that seems like logic that should not need to be implemented by each API consumer.
    #
    # Why is updateCampaign needed? Renaming campaigns requires it. Also, it is good practice for most API consumers to use a stable/immutable ID for a campaign (kind of like how we use the github numeric repo ID in our own DB to refer to GitHub repos).

    # TODO(sqs): Need to factor out input types more; there's a lot of duplication in these
    # mutations' params.

    # TODO(sqs): what is the lifecycle/lifetime of a CampaignUpdatePreview? It is what bundles together
    # multiple ChangesetSpecs so that you can have a short URL that shows the preview (otherwise the URL
    # would need to encode at least each ChangesetSpec ID). Or do we have a new type, such as a
    # ChangesetSpecList, that bundles these?

    # TODO(sqs): do we need 1 previewXyzCampaign mutation for each? or can we combine these into 1?
    previewCreateCampaign(namespace: ID!, spec: CampaignSpecInput!): CampaignUpdatePreview!
    previewApplyCampaign(namespace: ID!, spec: CampaignSpecInput!, lastSeq: Int): CampaignUpdatePreview!
    previewUpdateCampaign(campaign: ID!, spec: CampaignSpecInput!, lastSeq: Int): CampaignUpdatePreview!
    ...
    # Upload a changeset spec. In the createCampaign, applyCampaign, and updateCampaign mutations,
    # you can refer to changeset specs that were previously uploaded using this uploadChangesetSpec
    # mutation. That lets you avoid a very large request payload; instead, you can upload the
    # (possibly large) changeset specs individually.
    #
    # TODO(sqs): should we allow uploading changesetspecs that are not attached to a campaign? The
    # benefit is that we could get a truly atomic `src campaign apply` where it would not create a
    # campaign if uploading any of the changesetspecs failed beforehand. But it seems nice and
    # simple to always associate changesetspecs with a campaign, even if that means that the
    # campaign would sit around empty. That might even be the desired behavior, esp. since you
    # wouldn't create a bunch of duplicates if you retried because the name would be the same each
    # time.
    uploadChangesetSpec(campaign: ID!, changesetSpec: String!): ID!
...
input CampaignSpecInput {
    # The camaign spec YAML.
    campaignSpec: String!

    # Changeset specs that were computed locally. (TODO(sqs): what format? Richly-typed GraphQL, or
    # just raw JSON/YAML?) This field's items are merged with those from the changesetSpecIDs field.
    changesetSpecs: [String!]

    # Changeset specs that were previously uploaded using the uploadChangesetSpec mutation. This
    # field's items are merged with those from the changesetSpecs field.
    changesetSpecIDs: [ID!]
}
